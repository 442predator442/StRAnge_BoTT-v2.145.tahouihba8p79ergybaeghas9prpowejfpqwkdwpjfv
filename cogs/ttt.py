...
class tictactoe(commands.Cog):
    ...
    @commands.command(aliases = ['tic-tac-toe','t-t-t'])
    async def ttt(self, ctx, opponent : discord.Member):
        if ctx.author.bot is True:
            pass
        elif ctx.message.author == opponent:
            e = discord.Embed(
                description = "**<:xmark:731024248222777374>Вы не можете играть с собой.**",
            ...
            if ctx.guild.id in currentservers:
                e = discord.Embed(
                    description = f"**<:xmark:731024248222777374>Извините, но только одна игра в крестики-нолики может быть воспроизведена на сервере.**",
                    colour = discord.Colour.red()
                    )
                e.set_author(
                    name = f'{ctx.message.author.name}#{ctx.message.author.discriminator}',
                ...
                e = discord.Embed(
                    description = "**<:xmark:731024248222777374>Вы не можете играть с ботом.**",
                    colour = discord.Colour.red()
                    )
                e.set_author(
                    name = f'{ctx.message.author.name}#{ctx.message.author.discriminator}',
                ...
                def checkacceptance(message):
                    return message.author.id == opponent.id and  message.content == "+"
                try:
                    await ctx.send(f'{opponent.mention}, напишите `+` чтобы принять вызов {ctx.author.mention}')
                    await self.bot.wait_for('message', timeout = 30, check = checkacceptance)
                    currentservers.append(ctx.guild.id)
                    emm = discord.Embed(
                        title = 'Игра в крестики-нолики началась!',
                        colour = discord.Colour.green()
                        )
                    emm.add_field(name = 'Участники:', value = f'{ctx.author.mention} `против` {opponent.mention}')
                    ...
                    for variable in range(1,10):
                        if turn == x:
                            ...
                            try:
                                ...
                                eemb = discord.Embed(
                                    description = f"{y.mention} Не сумел использовать свой ход, {x.mention} является Победителем.",
                                    colour = discord.Colour.green()
                                    )
                                await ctx.send(embed = eemb)
                                currentservers.remove(ctx.guild.id)
                                break
                except asyncio.TimeoutError:
                    embb = discord.Embed(
                        description = f"{opponent.mention} Не принял ваш вызов.",
                        colour = discord.Colour.red()
                        )
                    embb.set_author(
                        name = f'{ctx.message.author.name}#{ctx.message.author.discriminator}',
    ...
    async def ttt_error(self, ctx, error):
        if isinstance(error, commands.MissingRequiredArgument):
            emb = discord.Embed(
                description = '**Введите все аргументы!**',
                colour = discord.Colour.red()
                )
            emb.add_field(name = 'Использовать:', value = '`ttt [@участник]`', inline = False)
            emb.add_field(name = 'Примеры:', value = '`ttt @напарник`', inline = False)
            emb.add_field(name = 'Права:', value = '`Участник сервера`', inline = False)
            emb.add_field(name = 'Псевдонимы:', value = '`t-t-t`, `tic-tac-toe`', inline = False)
